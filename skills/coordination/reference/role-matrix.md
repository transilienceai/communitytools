...
| **Validator — finding** | Coordinator | **Interleaved — the instant INTEGRATE materializes each candidate, before the next batch; re-spawned FRESH each cure round** | `finding_id`, FINDING_DIR, TARGET_URL, OUTPUT_DIR, frozen NVD/KEV snapshot | `attack-chain.md`, other findings, coordinator reasoning/theory, skill files (except VALIDATION.md), RESEARCH_BRIEF | `validated/{id}.json`, `false-positives/{id}.json`, `dropped/{id}.json`, `evidence/validation/` | Executor files (description.md, poc.py, poc_output.txt, evidence/raw-source.txt) | Terminal verdict written (CONFIRMED / REJECTED / DROPPED) |
| **Executor — cure** | Coordinator | On a DEMOTED verdict, before re-validation | ONLY `failed_checks` + `missing_evidence` + FINDING_DIR | `attack-chain.md`, coordinator theory, other findings, refuter/validator reasoning | `findings/finding-NNN/` (named gaps only) | Other agents' rows, validator artifacts, **re-theorizing** | Named gaps closed; loop re-validates on fresh blind agents |
| **Validator — engagement** | Coordinator | Once at loop end, over the interleaved `validated/` writes | OUTPUT_DIR root listing only | `attack-chain.md`, finding internals, validator reasoning | `engagement-validation.json`, `engagement-validation-summary.md` | All other files in OUTPUT_DIR | All thoroughness checks evaluated; PASS/FAIL |

## Why context separation matters

Validators must not see the coordinator's theory — that defeats independent validation, and they stay blind **every cure round** (re-spawned fresh, re-reading disk). The cure executor is handed ONLY the named `failed_checks`/`missing_evidence` and must **not re-theorize** — it closes exactly those gaps, so the re-validation's independence is preserved. The skeptic must not see the chain — that defeats anti-confirmation-bias. Explore executors get full chain because they need to know what's already tried; exploit executors additionally get scenarios because they need step-by-step recipes.
