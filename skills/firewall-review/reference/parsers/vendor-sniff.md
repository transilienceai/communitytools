...
**Reference implementation:** `fwrr.parsers._sniff.detect_vendor` in [firewall-review](https://github.com/ipunithgowda/firewall-review)
...
Priority order with confidence scores:

1. **PAN-OS XML** (1.00) — `<config version=` / `<panorama-config>` / `<rulebase>` (case-insensitive)
2. **Azure NSG** (0.95) — JSON shape with `securityRules` (top-level or under `properties`)
3. **AWS SG** (0.95) — JSON shape with `SecurityGroups`, or `IpPermissions` at top level, or `GroupId` + (`IpPermissions` or `IpPermissionsEgress`)
4. **FortiGate** (0.95) — `#config-version=FG` / `^config system global` / `config firewall policy` / `config vdom`
5. **Cisco ASA** (0.90) — `^ASA Version ` / `access-list <name> extended` (ASA explicitly wins over IOS)
6. **Cisco IOS** (0.85) — `^version \d+\.\d+` AND `ip access-list` AND NOT `^ASA Version `
7. **PAN-OS set** (0.85) — `^set rulebase security rules` / `^set devices configuration`
8. **iptables** (0.90) — `# Generated by iptables`, OR (`^*filter` AND `^-A <chain>`)

Anything else returns `("unknown", 0.0)`. Empty / unreadable / whitespace-only files also return `("unknown", 0.0)`.

## Inputs
- `path: pathlib.Path` — a single file. The function reads bytes itself; no streaming, no encoding parameter.
...
- **Backward-compat bypass.** Files inside a legacy per-vendor sub-directory (`cisco-asa/`, `cisco-ios/`, `fortigate/`, `palo-alto/`, `azure-nsg/`, `aws-sg/`, `iptables/` — see `_LEGACY_VENDOR_DIRS` in `launch.py`) are accepted on folder name alone and the sniff is **not** called. Old engagements pre-dating auto-detection keep working unchanged.
