...
1b. Try Direct Host Header Access FIRST (before any smuggling)
   └─ Discover internal hostnames from source, JS, headers
   └─ curl -H "Host: internal.hostname" http://target/path
   └─ Different content / flag? → Done, no smuggling needed
   └─ Same content / 403? → Proxy enforces ACLs, continue to smuggling

1c. Check for Internal Request Endpoints
   └─ Endpoints like /settings, /status, /health_check
   └─ POST to trigger → examine response for internal service output
   └─ These bypass the outermost proxy layer entirely
...
3b. Check for Proxy Defenses
   └─ Apache backend? → May send Connection: close when CL+TE both present
   └─ Reverse proxy rewrites Host? → Smuggled Host won't reach internal backend
       └─ Docker/container target? → Use docker exec/run --network to reach inner proxy directly
   └─ Multiple proxy layers? → Map each layer's TE/CL behavior independently

4. Choose Exploitation
   └─ Admin access needed? → Bypass controls
   └─ Internal backend? → Smuggle with Host: internal.hostname
...
## TE Parsing Behavior by Server

Understanding how each server parses `Transfer-Encoding` is critical for finding desync points:

| Server/Parser | TE Matching | Whitespace Tolerance | Rejects Unknown TE? |
|---------------|------------|---------------------|---------------------|
| Apache | Exact (case-insensitive) | Leading/trailing space+tab OK | Yes (400/501) + Connection: close if CL also present |
| Nginx | Exact (case-insensitive) | Rejects space before colon | Yes |
| HAProxy (HTX mode) | Exact | Space/tab OK | Strips CL when TE=chunked |
| HAProxy (legacy `no option http-use-htx`) | Exact | Space/tab/vtab OK | Keeps both CL+TE when TE unrecognized → **desync** |
| Python (mitmproxy, some libs) | **Substring** (`"chunked" in te`) | N/A (any value containing "chunked") | No — falls back to CL |
| Node.js (http) | Exact (last value wins) | Varies by version | Varies |
| IIS | Exact | Strict | Yes |
| h11 (Python) | Strict exact (`!= b"chunked"` → 501) | No tolerance | Yes |

**Key insight:** A desync occurs when one parser interprets the request as chunked and another uses Content-Length. The TE value must be accepted by one parser but rejected by the other.

---

## Reverse Proxy Host Rewriting

Some reverse proxies **always rewrite the Host header** when forwarding requests. This blocks smuggling-based internal backend access because the smuggled request's Host header gets overwritten.

**Detection:**
```bash
# Send a request with a random Host header
curl -sI http://target/ -H "Host: nonexistent.test.random"
# If response is identical to normal → proxy rewrites Host
```

**Affected proxies:** mitmproxy (reverse mode), Nginx with `proxy_set_header Host`, AWS ALB (sometimes), some CDNs

**Workarounds:**
1. Find an internal request endpoint (device status, health, webhook) that bypasses the outer proxy
2. Target an inner proxy layer if directly accessible (alternative ports, SSRF)
3. Use SSRF to reach internal services without going through the rewriting proxy
4. **Docker/container bypass** — if the target is a Docker container and you have CLI access, use `docker exec` or `docker run --network` to connect directly to the inner proxy from inside the Docker network, bypassing the Host-rewriting outer proxy entirely. Run CL.TE from there.
...
- **CVE-2021-41945**: h11 < 0.13.0 — accepts both CL+TE without rejection (Python HTTP parser)
- **CVE-2019-18277**: HAProxy < 2.0.6 — legacy mode (`no option http-use-htx`) keeps both CL+TE when TE is unrecognized, enabling CL.TE desync
...
- Tool: https://github.com//http-request-smuggler
