...
// that re-runs the documented prerequisites+steps and confirms (or corrects
// until it) reproduces the finding's stated result. Distinct from the refuters
// (which doubt) and the probe (which stats files): this one FOLLOWS THE RECIPE.
const REPRO_SCHEMA = {
  type: 'object', additionalProperties: true,
  required: ['reproduced'],
  properties: {
...
// prerequisites + step-by-step and the target, follows the recipe exactly, and
// confirms (or minimally corrects) that it reproduces the finding's result.
