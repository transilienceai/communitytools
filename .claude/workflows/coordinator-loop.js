...
// Which surface file this asset owns. A mobile app bundle is NOT an HTTP surface:
// handed an app dir, the web bootstrap below would manufacture a surface/v2 for an
// APK and THINK would then drive web attack classes at a binary. Renaming the file
// is not enough — the instruction block itself carries the web `type` enum and the
// web flag vocabulary, so it branches on the schema.
const SURFACE_FILE = a.surface_file || 'recon/inventory/surface.json'
const IS_MOBILE_SURFACE = /mobile-surface/.test(SURFACE_FILE)
const SURFACE_SCHEMA_NAME = IS_MOBILE_SURFACE ? 'mobile-surface/v1' : 'surface/v2'
...
const bootstrap = await agent(
  ...
  (MODE === 'coverage'
    ? (IS_MOBILE_SURFACE
      ? `6. COVERAGE (deterministic, MOBILE APP BUNDLE): (a) The surface file is OUTPUT_DIR/${SURFACE_FILE} — schema ${SURFACE_SCHEMA_NAME}. It was written by tools/mobile_surface_build.py in the Acquire phase: PRESERVE IT. Do NOT re-enumerate it, do NOT write a surface/v2 here, and NEVER treat this asset as an HTTP surface — an app bundle has no listener, and the backend it talks to is a SEPARATE asset with its own surface.json and its own coverage. If (and only if) you discover a genuinely new app unit at runtime, APPEND it: {"unit_id":...,"type":"component|deeplink|webview|storage|crypto-use|other","address":<component name / container path / call-site — never parsed, a report anchor only>,"flags":[<ONLY the mobile flags: local_store,crypto_use,exported_component,webview>],"equiv_group":<null OR a short id shared ONLY by near-identical components — assign CONSERVATIVELY; the gate CREDITS siblings from it, so an invented group erases cells>,"evidence_ref":["E-NNN"]}. (b) write OUTPUT_DIR/coverage.json = one row per class_id {class_id, applicability, status, units_tested:[]}. The catalog is skills/coordination/reference/coverage-matrix.json; code (tools/enumerate_cells.py + tools/coverage_gate.py) computes the work-list from the surface file — you supply FACTS, code enforces coverage. (c) There is NO mechanical passive pass for a binary: passive_web_probe.py is an HTTP tool and must NOT be run against an app bundle. Close cells with real static and runtime evidence instead — per-class technique detail is in skills/mobile-security/reference/masvs-class-map.md. NOTE proof_mode: a cell the gate marks [runtime] cannot be closed from the artifact; it needs a running instance, and a proof_mode:static cell can NEVER be device-deferred. Matrix hint:\n${COVERAGE_MATRIX ? JSON.stringify(COVERAGE_MATRIX) : 'READ skills/coordination/reference/coverage-matrix.json and instantiate its full class list.'}\n`
      : `6. COVERAGE (deterministic): (a) ENUMERATE the discovered asset surface into OUTPUT_DIR/${SURFACE_FILE} — schema ${SURFACE_SCHEMA_NAME}: {"schema":"surface/v2","asset_tag":<the OUTPUT_DIR basename>,"apex":<apex or "">,"units":[{"unit_id":"u-0001","type":"page|endpoint|param|form|origin|host|port|service","address":<url or host:port>,"methods":[...],"flags":[<ONLY agent-set coverage flags that the observation warrants: object_by_id,json_body,role_verb_gated,sensitive_flow,server_fetched_url,input_sink,workflow,deser_or_ci,inbound_webhook,id_keyed_unauth,stored_field,auth_surface,consumes_upstream,static_js_or_repo>],"evidence_ref":["E-NNN"],"equiv_group":<null OR a short group id shared ONLY by cells with the SAME route template / handler / param family (e.g. one id for /users/{id} across ids) — assign CONSERVATIVELY; when unsure, null>}]}. RESUME-AWARE: if OUTPUT_DIR/${SURFACE_FILE} AND OUTPUT_DIR/coverage.json BOTH already exist (a resume run, or a surface recovered from a mobile bundle), PRESERVE both verbatim — do NOT re-enumerate or overwrite — and skip straight to (c); if exactly one of the two exists, generate the missing one from the present one WITHOUT discarding the present file. (b) write OUTPUT_DIR/coverage.json = one row per class_id {class_id, applicability, status, units_tested:[]}. The catalog is skills/coordination/reference/coverage-matrix.json (machine) / .md (human); code (tools/enumerate_cells.py + tools/coverage_gate.py) computes applicability and the work-list from your surface.json — you supply the FACTS, code enforces coverage. (c) DETERMINISTIC MECHANICAL COVERAGE (E1 tools-not-agents): run \`python3 tools/passive_web_probe.py --asset-dir OUTPUT_DIR --allow <the in-scope host(s) from surface.json>\` — one non-destructive pass that clears the mechanical attack-classes (TLS posture, security headers, HTTPS downgrade/HSTS, verbose errors, CORS, vulnerable components, secret-exposure, inventory) as tool-corroborated covered_negative cells and READ-MERGES OUTPUT_DIR/coverage.json (it never clobbers). Then the reasoning loop only chases the harder classes. Matrix hint:\n${COVERAGE_MATRIX ? JSON.stringify(COVERAGE_MATRIX) : 'READ skills/coordination/reference/coverage-matrix.json and instantiate its full class list.'}\n`)
    : ``) +
